...
def selenium_task(worker, data):
    ...
    while stateProdSizeBtn == False:
        try:
            if check_exists_by_css_selector("#pdp-store-stock-checker-app-container > div > aside > div > section > form > div.size-selector > ul > li:nth-child(2) > button", worker) == True:
                product_size_btn = WebDriverWait(worker, 20).until(ec.visibility_of_element_located((By.CSS_SELECTOR,'#pdp-store-stock-checker-app-container > div > aside > div > section > form > div.size-selector > ul > li:nth-child(2) > button')))
                product_size_btn.click()
                stateProdSizeBtn = True
                print("\nProduct size selected.\n")
            else:
                select = Select(worker.find_element_by_css_selector('#pdp-store-stock-checker-app-container > div > aside > div > section > form > div.size-selector > div > select'))
    ...
    while True:
        try:
            WebDriverWait(worker, 20).until(ec.element_to_be_clickable((By.CSS_SELECTOR, '#pdp-store-stock-checker-app-container > div > aside > div > section > section:nth-child(5) > button'))).click()
